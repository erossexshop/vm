/* ═══════════════════════════════════════════════════════════════════════
   ...
   id | name | cat | price | img | activo | desc | marca | featured
   ═══════════════════════════════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════════════════════════════
   CONVERSIÓN  —  filas CSV → objetos de producto (9 columnas)
   ══════════════════════════════════════════════════════════════════════ */
function rowsToProducts(rows) {
  ...
  const toFeat = v => String(v).trim().toLowerCase() === 'si';

  return rows.slice(1).map(r => {
    const get = idx => (idx >= 0 && idx < r.length) ? r[idx] : '';
    const rawCat = get(IDX.cat) || 'General';
    const cats   = rawCat.split('|').map(c => c.trim()).filter(Boolean);
    return {
      id      : toNum(get(IDX.id)),
      name    : get(IDX.name)    || '(sin nombre)',
      cat     : cats[0],          /* categoría principal (para mostrar) */
      cats    : cats,             /* todas las categorías (para filtrar) */
